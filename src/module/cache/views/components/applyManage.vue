	...
	<div id="db_app">
		...
		<div>
			<paper>
				...
				<el-row>
          <el-button class="button is-info pull-left" @click="applyCache()">申请</el-button>
          ...
          <el-col style="float:right;width: 20%">
            <search-inp @search-enter="getApplyListFun(searchVal,1,data.page.pageSize)" :placeword="'请输入应用名称'" v-model="searchVal"></search-inp>
				...
				<div class="container" style="margin-top: 20px">
					<full-table :data="data.data" :title="data.tableTitle" :columnsOperation="data.columnsOperation" :page="data.page" @size-change="sizeChange" @current-change="currentChange"></full-table>
				</div>
			</paper>
		</div>
		
		<el-dialog :close-on-click-modal="false" id="detailDialog" size="large" title="缓存详情" :visible.sync="detailDbDialog">
			...
			<div id="applyDialogTitle">资源申请（云主机+缓存）</div>
			<div id="applyList">
  ...
  export default{
    ...
    data(){
      return {
				...
				dialogData:{
					...
					submitDbList:[//提交缓存申请数据
						{addRow:{region:'',cluster:'',version:'',hostConfig:'',flavorId:'',cpu:'',memory:'',hardware:'',operatSystem:'',clusterDesc:''}}
					],
				...
				groupList:[
					{name:'单节点',id:'redis_01'},
					{name:'一主一从',id:'redis_02'},
					{name:'三节点哨兵高可用架构',id:'redis_03'},
					{name:'三节点coids分布式架构',id:'redis_04'},
				],
    ...
    created(){
		...
		methods:{
			...
			applyCache:function(){
				this.$router.push('/cache/apply');
			},

			//刷新
			refreshFun:function(){
				this.getApplyListFun(this.currentSearchVal,1,this.data.page.size);
			},
