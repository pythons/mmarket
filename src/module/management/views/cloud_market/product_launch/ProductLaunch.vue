                ...
                <el-row>
                    <el-col :span="24">
                        <table class="detailTable" width="100%" border="1">
                            ...
                            <th><span @click="addItem" class="addItem mf mf-lg mf-add"></span></th>
                            </thead>
                            <tbody>
                            <tr align="center" v-for="(item,key) in versions" :key="key">
                                <td>
                                  <el-input  :maxlength="6" placeholder="请输入版本号(限6个字符)" size="small" class="VersionInput" type="text"
                                              v-model='item.seq'></el-input>
                                </td>
                                <td>
                                    <el-input :maxlength="30"  size="small" class="VersionInput" type="text"
                                ...
                                <td><span @click="deleteItem(item.id)" class="deleteItem mf mf-lg mf-shanchu"></span>
                                </td>
                            </tr>
                            </tbody>
                        </table>
  ...
  export default {
    data(){
      return {
        ...
        //---标题的变化
        titles:this.$route.params.productId==='-999'?{titleName:'产品发布'}:{titleName:'产品修改'},
        content:"",  
        ...
        rules: {
          ...
          downloadURL:[
            {required: true, message: '请输入下载地址信息',trigger: 'blur'},
    ...
    created(){
      if(this.$route.params.productId==='-999'){
     ...
     submitForm(){
       ...
       this.$refs["ruleForm"].validate((valid) => { 
          ...
          let params = {
            ...
            productId:that.$route.params.productId==='-999'?-1:that.$route.params.productId,
     ...
     getEditorDatas(){
        srv.requestProductDetails({productId:this.$route.params.productId}).then(res =>{
          if(res.code === 200) {
               ...
               this.imageUrl1 = res.result.logo.split(",")[0];
               this.imageUrl2 = res.result.logo.split(",")[1];
               }

               this.ruleForm.operateSystem = res.result.supportOs;
               this.ruleForm.compatibility = res.result.compatibility;
              
               axios.get(res.result.description.slice(23)).then(result=>{
