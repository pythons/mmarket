  ...
  <section class="container-header">
    ...
    <section class="nav hidden-xs">
      <el-dropdown v-for="moduler in menuList" :class="{'head-nav': true}" menu-align="star" @command="navigate">
        <section class="header-name" ><a :href="moduler.link">{{moduler.name}}</a></section>
    ...
    <section class="other">
      ...
      <section class="setting">
        ...
        <div class="header-icon" title="消息中心"><i class="mf mf-lg mf-xiaoxizhongxin"></i><badge v-model="badge"></badge></div>
  ...
  export default{
    ...
    methods: {
      ...
      getMenuList() {
        ...
        srv.getSystemMenu(params).then((res) => {
       ...
       * */
      getRole(project) {
        let lsUser = ss.getObject(['user']);
        switchProject({projectId: project.projectId}).then(data => {
          if(data.errorCode == '200') {
            srv1.getUserRole(true,lsUser.userId, project.projectId).then(data => {
              if (data.errorCode == '200') {
